...
class CropData:
    def __init__(self, input_dir, output_dir, segment_name, check_geography):
    ...
    def check_geography(self):
        ...
        land_use = self.read_csv(LAND_USE)
        land_use = land_use[["maz", "taz"]]
        land_use = land_use.sort_values(["taz", "maz"])

        households = self.read_csv(HOUSEHOLDS)
        ...
        if len(orphan_households) > 0:
            ...
            print(
                f"writing {file_name} {orphan_households.shape} to {self.input_path(file_name)}"
            )
            orphan_households.to_csv(self.input_path(file_name), index=False)
        ...
        maz = self.read_csv(MAZ_TAZ).sort_values("MAZ")
    ...
    def crop_data(self):
        ...
        slicer = segments[self.segment_name]
        ...
        self.to_csv(land_use, "land_use.csv")
    ...
    def crop_omx(self, omx_file_name, zones, num_outfiles=1):
        ...
        # create
        if num_outfiles == 1:
            omx_out = [omx.open_file(self.output_path(f"{omx_file_name}.omx"), "w")]
        else:
            omx_out = [
                omx.open_file(self.output_path(f"{omx_file_name}{i + 1}.omx"), "w")
    ...
    def patch_maz(self, df, maz_offset):
        for c in df.columns:
    ...
    def read_csv(self, file_name):
        df = pd.read_csv(self.input_path(file_name))
...
if __name__ == "__main__":
    ...
    CropData(input_dir, output_dir, segment_name, check_geography)
